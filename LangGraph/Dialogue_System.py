...
# 初始化Tavily客户端（API key 可选）
tavily_api_key = os.getenv("TAVILY_API_KEY")
tavily_client = TavilyClient(api_key=tavily_api_key) if tavily_api_key else None

# 这是对话系统的第一个节点——理解与查询节点
...
def tavily_search_node(state: SearchState) -> SearchState:
    ...
    # 如果没有配置 Tavily API key，跳过搜索
    if tavily_client is None:
        print("⚠️ 未配置 TAVILY_API_KEY，跳过搜索")
        return {
            "search_results": "",
            "step": "search_failed",
            "messages": [AIMessage(content="⚠️ 未配置搜索API，将基于已有知识为您回答")]
        }
    ...
    while True:
        ...
        try:
            ...
            async for output in app.astream(initial_state, config=config):
                # output是字典，key是节点名称，value是节点输出
                for node_name, node_output in output.items():
                    if "messages" in node_output and node_output["messages"]:
                        latest_message = node_output["messages"][-1]
                        # 判断 latest_message 是否是 AIMessage 类型的实例。
